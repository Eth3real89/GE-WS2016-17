...
public class EnviroVegetationInstance : MonoBehaviour {
	...
	void Start () 
	{
		EnviroMgr.instance.RegisterMe (this);
		currentSeason = EnviroMgr.instance.seasons.currentSeasons;
		maxAgeInHours = EnviroMgr.instance.GetInHours (Age.maxAgeHours, Age.maxAgeDays, Age.maxAgeYears);
	...
	{
		ageInHours = EnviroMgr.instance.currentTimeInHours - Age.birthdayInHours;
		KeepVariablesClear ();
		if (!stay) {
	...
	{
		Age.birthdayInHours = EnviroMgr.instance.currentTimeInHours - startAge;
		startAge = 0f;
		ageInHours = 0f;
		currentStage = stage;
		rescale = true;
